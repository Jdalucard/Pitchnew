...
import {
  IFilterMediaOutletsSearchsOptions,
  MediaOutletItem,
  MediaOutletsSearchFiltering,
} from './components';
import { magazinePositions, mediaOutletCategories } from '../../../constants';
import { processGetMediaOutlets } from '../../../redux/searches/mediaOutletsSearch';
import { processGetTotalEvents } from '../../../redux/searches/eventsSearch';
import { saveUserSearch } from '../../../redux/searches/userSavedSearches';

export interface IMediaOutletResult {
  _id: string;
  companyName: string;
  description: string;
  firstName: string;
...
export function MediaOutletsSearch() {
  ...
  const [selectedItems, setSelectedItems] = useState<IMediaOutletResult[]>([]);
  ...
  const getContactListInfoIfExists = useCallback(
    (results: IMediaOutletResult[]) => {
      const itemsThatMatch: IMediaOutletResult[] = [];
      const itemsThatDontMatch: IMediaOutletResult[] = [];
  ...
  const getMediaOutlets = useCallback(
    async (
      offset: number,
      isResettingValue: boolean,
      filters?: IFilterMediaOutletsSearchsOptions,
    ) => {
      setIsLoadingResults(true);

      const searchParameters: ISearchTransformedParameters = {
        mediaType: filters?.type.value ?? mediaOutletCategories.magazine,
        resultsPerPage: resultsPerPage,
        offset,
      };

      const isMagazine = searchParameters.mediaType === mediaOutletCategories.magazine;
      if (isMagazine) {
        searchParameters.position = filters?.position?.value ?? magazinePositions.editor;
      }
      ...
      if (filters) {
        ...
        if (state.value !== 'all' && !isMagazine) searchParameters.state = state.value;
        if (city.value !== 'all' && !isMagazine) searchParameters.city = city.label;
      }

      const formattedSearchParameters = formatQueryParameters(searchParameters);

      const response = await dispatch(processGetMediaOutlets(formattedSearchParameters)).unwrap();
      const totalResponse = await dispatch(processGetTotalEvents()).unwrap();

      if (response?.results && totalResponse?.total) {
        setCurrentResults((prev) => {
          let newResults: IMediaOutletResult[];
        ...
        if (isResettingValue) {
          dispatch(
            saveUserSearch({
              type: contactSearchTypes.mediaOutletSearch,
  ...
  const handleProcessFiltering = (filters: IFilterMediaOutletsSearchsOptions) => {
    getMediaOutlets(0, true, filters);
  };
  ...
  const handleToggleLoadingView = (view: 'loadMore' | 'pagination') => {
    ...
    if (currentResults.offset > resultsPerPage * 1) {
      getMediaOutlets(0, true);
    }
  };

  const handleToggleSelectAll = () => {
    let newItemsSelected: IMediaOutletResult[] = [];
  ...
  const handleItemSelection = (item: IMediaOutletResult) => {
  ...
  const handleLoadMore = () => {
    getMediaOutlets(currentResults.offset, false);
  };

  const handlePaginationChange = (_e: React.ChangeEvent<unknown>, page: number) => {
    getMediaOutlets(page * resultsPerPage - resultsPerPage * 1, true);
  ...
  return (
    <>
      <div className={styles.contactSearchesWrapper}>
        ...
        <MediaOutletsSearchFiltering handleProcessFiltering={handleProcessFiltering} />
                ...
                <>
                  {loadingView === 'loadMore' ? (
                    ...
                    <Pagination
                      ...
                      shape="rounded"
                      color="primary"
                      count={90} // fixed max fot the media outlets
