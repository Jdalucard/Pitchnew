...
export function PodcastItem({ selectedItems, itemInfo, handleItemSelection }: IProps) {
    ...
    <div className={`${styles.searchItem} ${itemIsSelected() ? styles.itemSelected : ''}`}>
      ...
      <div className={styles.textDescriptionWrapper}>
        ...
        <Box sx={(theme) => ({ fontSize: '0.8rem', color: theme.palette.text.secondary })}>
          <ReactMarkdown>{convertToMarkdown(trimString(itemInfo.description, 300))}</ReactMarkdown>
        </Box>
        {!!itemInfo.tags?.length && (
          <div className={styles.listTagsWrapper}>
            {itemInfo.tags.map((tag) => {
              return (
                <div key={tag.listId} className={styles.listTag}>
                  <Typography variant="caption" color="text.primaryInverted">
                    {tag.listName}
                  </Typography>
                </div>
              );
