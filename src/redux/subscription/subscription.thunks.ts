...
export const paySubscription = createAsyncThunk(
  ...
  async (params: IPaySubscription, thunkApi) => {
    ...
    try {
      ...
      thunkApi.dispatch(errorAlert('Error processing the payment for the plan selected. Please, try again later.'));
    }
  }
);
...
export const payBundle = createAsyncThunk(
  ...
  async (bundleType: string, thunkApi) => {
    try {
      ...
      thunkApi.dispatch(errorAlert('Error processing the payment for the bundle selected. Please, try again later.'));
    }
  }
);
...
export const getUserSubscriptionPlan = createAsyncThunk(
  ...
  async (_, thunkApi) => {
    try {
      ...
      thunkApi.dispatch(errorSideAlert('Error getting the user subscription plan. Please, try again later.'));
    }
  }
);
...
export const updateUserSubscriptionPlan = createAsyncThunk(
  ...
  async (newPlanId: string, thunkApi) => {
    try {
      const response = await axios.put(`${subscriptionPath}/current`, { planId: newPlanId });

      return response.data;
    } catch (error) {
      thunkApi.dispatch(errorAlert('Error updating the user subscription plan. Please, try again later.'));
    }
  }
);
...
export const addUserSubscriptionPlan = createAsyncThunk(
  ...
  async (params: IAddUserSubscriptionPlan, thunkApi) => {
    ...
    try {
      const response = await axios.post(`${subscriptionPath}/addSubscription`, transformedParamsForBe);
...
export const addSubscriptionPlanToStripe = createAsyncThunk(
  ...
  async (newPlan: IAddSubscriptionPlanToStripe, thunkApi) => {
    try {
      ...
      thunkApi.dispatch(errorAlert('Error adding the new subscription plan to Stripe. Please, try again later.'));
    }
  }
);
...
export const getStripeSubscriptionPlansData = createAsyncThunk(
  ...
  async (_, thunkApi) => {
    try {
      ...
      thunkApi.dispatch(errorAlert('Error getting subscription plans data from Stripe. Please, try again later.'));
    }
  }
);